...
# Helper function to get SVM prediction
def get_svm_prediction(user_id, wpm, accuracy, streak, session_history):
    """Get SVM-based difficulty prediction"""
    try:
        from svm_engine import extract_features, _get_or_load, CLASS_TO_DIFFICULTY, CLASS_LABELS, phoneme_score
        ...
        return {'svm_available': False, 'error': str(e)}
...
class LearningSession(db.Model):
    ...
    time_spent = db.Column(db.Integer, nullable=False)  # stored in SECONDS
...
def speech_test():
    try:
        ...
        # Get SVM prediction for enhanced difficulty adjustment
        recent_tests = TestResult.query.filter_by(user_id=user_id).order_by(TestResult.created_at.desc()).limit(5).all()
        session_history = [t.accuracy for t in reversed(recent_tests)]
        
        # Calculate current streak
        current_streak = 0
        for test in recent_tests:
            if test.accuracy >= 0.7:  # Consider 70%+ as "correct"
                current_streak += 1
            else:
                break
        
        svm_prediction = get_svm_prediction(user_id, len(spoken_words) * 2, accuracy, current_streak, session_history)
        ...
        # Add SVM prediction data if available
        if svm_prediction.get('svm_available'):
            response_data['svm_prediction'] = svm_prediction
...
def listening_test():
    try:
        ...
        svm_prediction = get_svm_prediction(user_id, 0, accuracy, current_streak, session_history)  # wpm=0 for listening
...
def get_parent_dashboard(parent_id):
    try:
        ...
        for child in children:
            ...
            # ── Group seconds by date → convert to minutes ──────────────────
            activity_by_date_child = {}
            for session in sessions:
                activity_by_date_child[session.date] = (
                    activity_by_date_child.get(session.date, 0) + session.time_spent
                )

            learning_dates = sorted(activity_by_date_child.keys(), reverse=True)
            ...
            today_seconds_child = activity_by_date_child.get(today_date, 0)
            today_learning_child = today_seconds_child // 60  # convert to minutes
            ...
            child_data = {
                ...
                'today_learning': today_learning_child,
                'highest_scores': {
...
def get_dashboard_data(user_id):
    try:
        ...
        # ── FIX: accumulate raw seconds per day, then convert to minutes ────
        activity_by_date = {}  # date -> total seconds
        for session in sessions:
            activity_by_date[session.date] = (
                activity_by_date.get(session.date, 0) + session.time_spent
            )

        # Convert every entry from seconds → minutes (integer floor)
        activity_by_date_minutes = {
            d: total_seconds // 60
            for d, total_seconds in activity_by_date.items()
        }

        today = date.today()

        # ── Streak (uses date presence, not minutes value — unchanged) ───────
        current_streak = 0
        check_date = today
        ...
        # ── Today's learning in minutes ──────────────────────────────────────
        today_learning = activity_by_date_minutes.get(today, 0)

        # ── Learning blocks (calendar) ───────────────────────────────────────
        end_date = today
        ...
        for i in range(150):
            ...
            minutes = activity_by_date_minutes.get(block_date, 0)  # ✅ already in minutes
            ...
            learning_blocks.append({
                ...
                'minutes': minutes,          # ✅ correct minutes value
                'intensity': intensity,
                'filled': minutes > 0
            })
...
if __name__ == '__main__':
    ...
    try:
        register_svm(app)
        print("✓ SVM Engine registered successfully")
    except Exception as e:
        print(f"⚠ SVM Engine registration failed: {e}")
